...
相比于其他复杂的数据结构，并查集并没有复杂的结构，同时也拥有着优秀的时间和空间复杂度，代价是能做的事局限于查询和合并等基础操作。

### [P128 最长连续序列（](https://leetcode.com/problems/longest-consecutive-sequence/)中等）

并查集思想的巧妙运用，实际上对于这种能够合并的（在此题当中是同一段线段）我们都可以考虑使用并查集的结构。在这道题中，我们就只需要统计祖先（线段的右端点）的长度，统计长度就回推就好了。我们还可以使用压缩路径的方式使得空间复杂度和时间复杂度进一步降低。
