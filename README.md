...
构造一个随机的至多 `MAXLEVEL` 层（一般为 32 层）的链表，使得最底层的链表是有序列表，并且每一个底层的元素以一个固定的概率 $p$ 出现在顶上一层，那么我们在搜索 `a` 的时候就可以通过从顶上搜索到最大的小于 `a` 的元素之后变到下一层继续进行搜索。可以证明空间复杂度是 $O(n)$ 的，同时查询等操作都是 $O(\log n)$ 的。
...
1. 每日挑战一定要做
2. 尝试使用 Python 写题，用来复习
3. 每日坚持练习新的和困难的知识点

### [P71 简化路径](https://leetcode.com/problems/simplify-path/)（中等）

真的好简单，构造目录的栈就行。

### [P42 捕获雨水](https://leetcode.com/problems/trapping-rain-water/)（困难）

考虑维持高度的栈，栈顶的高度最低，一旦弹出就计算累计的雨水，相当于把雨水从横向的角度一层一层分解，不同于竖向的分解，分解为竖向的时候我们比较难判断这个地方的高度是多少，这样非常慢。

### [P726 原子个数](https://leetcode.com/problems/number-of-atoms/)（困难）
...
从今天开始，可能我会将每日挑战我觉得有意思的题目写一写感想，但是不管写不写我都会把相应代码放到本仓库的 [DailyChallenge](.\DailyChallenge) 文件夹中。

### [P1202 最小交换字符串](https://leetcode.com/problems/smallest-string-with-swaps/)（中等）
...
### [P51 N 皇后](https://leetcode.com/problems/n-queens/)（困难）
...
### [P37 数独](https://leetcode.com/problems/sudoku-solver/)（困难）
