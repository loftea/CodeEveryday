...
本质是为了要找到每个被限制的建筑“真正”被限制到了什么高度。当然有一个暴力的做法，是 $O(n)$ 的，但是这显然不好。我们就要想怎么去做这件事情。如果我们每次都对最低的那个限制考虑其周围的限制建筑，那么就可以了，关键是要维持住最值结构，显然就要使用堆。

注意剪枝，已经操作过的限制建筑无需再操作。实际操作上，我们 `push()` 被修剪过的限制，这个限制比原来要小，所以会先出队列，所以不用担心会有非真实限制的建筑进行修剪。
